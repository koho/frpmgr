...
func (pd *EditProxyDialog) pluginProxyPage() TabPage {
	return TabPage{
		...
		Children: []Widget{
			...
			NewBrowseLineEdit(nil, Bind("vm.PluginUnixVisible"), Bind("PluginUnixPath"),
				"选择 Unix 路径", "", true),
			Label{Visible: Bind("vm.PluginStaticVisible"), Text: "本地路径:"},
			NewBrowseLineEdit(nil, Bind("vm.PluginStaticVisible"), Bind("PluginLocalPath"),
				"选择本地文件夹", "", false),
			...
			NewBrowseLineEdit(nil, Bind("vm.PluginCertVisible"), Bind("PluginCrtPath"),
				"选择证书文件", "证书文件 (*.crt, *.cer)|*.crt;*.cer|", true),
			Label{Visible: Bind("vm.PluginCertVisible"), Text: "密钥路径:"},
			NewBrowseLineEdit(nil, Bind("vm.PluginCertVisible"), Bind("PluginKeyPath"),
				"选择密钥文件", "密钥文件 (*.key)|*.key|", true),
